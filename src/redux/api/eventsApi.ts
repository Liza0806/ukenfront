...
import { AddEventTypeDB, EventTypeDB } from "../types/types";
...
export const getAllEvents = async () => {
  ...
  export const addEventAPi = async (event: AddEventTypeDB)=>{
    try {
      console.log(event, 'event in addEventAPi')
      const response = await axios.put(`/events/`, { date: event.date, groupId: event.groupId, groupTitle: event.groupTitle, isCancelled: event.isCancelled, participants: event.participants});
   //   const response = await axios.post(`https://ukenback.vercel.app/events/`, {date: event.date, groupId: event.groupId, groupTitle: event.groupTitle, isCancelled: event.isCancelled, participants: event.participants});
     // const response = await axios.put(`/events/${event._id}`, {_id: event._id, date: event.date, groupId: event.groupId, groupTitle: event.groupTitle, isCancelled: event.isCancelled, participants: event.participants});
