...
describe("Функция handleSubmit", () => {
  it("успешно добавляет новую группу и обновляет состояние в Redux", async () => {
    ...
    await handleSubmit(false, mockGroupFormState, store.dispatch);
  ...
  it("успешно обновляет группу и обновляет состояние в Redux", async () => {
    // @ts-ignore
    mockedupdateGroup.mockResolvedValue({ updatedGroup: mockUpdateGroup, _id: '1' });
   // mockedupdateGroup.mockResolvedValue({status: 200, data: {mockUpdateGroup, id}})
 // Вызываем handleSubmit с реальным dispatch
 await handleSubmit(true, mockUpdateGroupFormState, store.dispatch, initialGroupData);
